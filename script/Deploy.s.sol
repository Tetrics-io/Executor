...
contract DeployScript is Script {
    ...
    function _deployHyperliquidAdapters(address deployer) internal {
        ...
        if (protocols.hypeStaking != address(0)) {
            ...
            if (existingHypeStaking != address(0)) {
                ...
                stakedHypeAdapter = new StakedHypeAdapter(address(hyperliquidExecutor));
        ...
        if (protocols.hyperlendPool != address(0)) {
            ...
            if (existingHyperLend != address(0)) {
                ...
                hyperlendAdapter = new HyperLendAdapter(address(hyperliquidExecutor));
        ...
        if (protocols.felixCdp != address(0)) {
            ...
            if (existingFelix != address(0)) {
                ...
                felixAdapter = new FelixAdapter(address(hyperliquidExecutor));
        ...
        if (protocols.hyperbeatVault != address(0)) {
            ...
            if (existingHyperBeat != address(0)) {
                ...
                hyperbeatAdapter = new HyperBeatAdapter(address(hyperliquidExecutor));
    ...
    function _initializeSystem() internal {
        ...
        // Register Ethereum protocols if deployed
        if (address(lidoAdapter) != address(0) && address(executor) != address(0)) {
    ...
    function _updateEnvironmentVariables() internal {
        ...
        if (address(multiSig) != address(0)) console.log("MULTISIG_ADDRESS=%s", address(multiSig));
        if (address(oracle) != address(0)) console.log("ORACLE_ADDRESS=%s", address(oracle));
        if (address(priceValidator) != address(0)) console.log("PRICE_VALIDATOR_ADDRESS=%s", address(priceValidator));
        if (address(executor) != address(0)) console.log("EXECUTOR_ADDRESS=%s", address(executor));
        if (address(lidoAdapter) != address(0)) console.log("LIDO_ADAPTER_ADDRESS=%s", address(lidoAdapter));
        if (address(wstethAdapter) != address(0)) console.log("WSTETH_ADAPTER_ADDRESS=%s", address(wstethAdapter));
        if (address(morphoAdapter) != address(0)) console.log("MORPHO_ADAPTER_ADDRESS=%s", address(morphoAdapter));
        if (address(acrossAdapter) != address(0)) console.log("ACROSS_ADAPTER_ADDRESS=%s", address(acrossAdapter));
        if (address(hyperliquidExecutor) != address(0)) console.log("HYPERLIQUID_EXECUTOR_ADDRESS=%s", address(hyperliquidExecutor));
        if (address(stakedHypeAdapter) != address(0)) console.log("HYPE_STAKING_ADAPTER_ADDRESS=%s", address(stakedHypeAdapter));
        if (address(hyperlendAdapter) != address(0)) console.log("HYPERLEND_ADAPTER_ADDRESS=%s", address(hyperlendAdapter));
        if (address(felixAdapter) != address(0)) console.log("FELIX_ADAPTER_ADDRESS=%s", address(felixAdapter));
        if (address(hyperbeatAdapter) != address(0)) console.log("HYPERBEAT_ADAPTER_ADDRESS=%s", address(hyperbeatAdapter));
    }
    ...
    function _printDeploymentSummary() internal view {
        ...
        console.log("\\nCORE CONTRACTS:");
        if (address(multiSig) != address(0)) console.log("  MultiSig: %s", address(multiSig));
        if (address(oracle) != address(0)) console.log("  Oracle: %s", address(oracle));
        if (address(priceValidator) != address(0)) console.log("  PriceValidator: %s", address(priceValidator));
        if (address(executor) != address(0)) console.log("  Executor: %s", address(executor));
        
        console.log("\\nETHEREUM ADAPTERS:");
        if (address(lidoAdapter) != address(0)) console.log("  LidoAdapter: %s", address(lidoAdapter));
        if (address(wstethAdapter) != address(0)) console.log("  WstETHAdapter: %s", address(wstethAdapter));
        if (address(morphoAdapter) != address(0)) console.log("  MorphoAdapter: %s", address(morphoAdapter));
        if (address(acrossAdapter) != address(0)) console.log("  AcrossAdapter: %s", address(acrossAdapter));
        
        if (address(hyperliquidExecutor) != address(0)) {
            console.log("\\nHYPERLIQUID CONTRACTS:");
            console.log("  HyperliquidExecutor: %s", address(hyperliquidExecutor));
            if (address(stakedHypeAdapter) != address(0)) console.log("  StakedHypeAdapter: %s", address(stakedHypeAdapter));
            if (address(hyperlendAdapter) != address(0)) console.log("  HyperLendAdapter: %s", address(hyperlendAdapter));
            if (address(felixAdapter) != address(0)) console.log("  FelixAdapter: %s", address(felixAdapter));
            if (address(hyperbeatAdapter) != address(0)) console.log("  HyperBeatAdapter: %s", address(hyperbeatAdapter));
        }
