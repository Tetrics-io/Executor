...
contract Permit2ContractWalletTest is Test {
    ...
    function test_UnauthorizedEOAPermit2Validation() public {
        ...
        assertFalse(
            executor.isAuthorizedForPermit2(testHash, unauthorized),
            "Unauthorized address should not be authorized for Permit2"
        );
    }
    ...
    function test_ContractWalletPermit2Validation() public {
        ...
        assertFalse(
            executor.isAuthorizedContractWallet(testHash, address(contractWallet)), "Hash should not be valid initially"
        );
        assertFalse(
            executor.isAuthorizedForPermit2(testHash, address(contractWallet)),
            "Contract wallet should not be authorized initially"
        );
        ...
        assertTrue(
            executor.isAuthorizedContractWallet(testHash, address(contractWallet)), "Hash should be valid after setting"
        );
        assertTrue(
            executor.isAuthorizedForPermit2(testHash, address(contractWallet)),
            "Contract wallet should be authorized after validation"
        );
    }
    ...
    function test_InvalidContractWalletPermit2Validation() public {
        ...
        assertFalse(
            executor.isAuthorizedContractWallet(testHash, address(invalidWallet)),
            "Invalid wallet should not be authorized"
        );
        assertFalse(
            executor.isAuthorizedForPermit2(testHash, address(invalidWallet)),
            "Invalid wallet should not be authorized for Permit2"
        );
    }
    ...
    function test_ApprovedSolverPermit2Validation() public {
        ...
        assertFalse(
            executor.isAuthorizedForPermit2(testHash, newSolver),
            "New solver should not be authorized for Permit2 initially"
        );
        ...
        assertTrue(
            executor.isAuthorizedForPermit2(testHash, newSolver),
            "New solver should be authorized for Permit2 after approval"
        );
    }
    ...
    function test_EmergencyOperatorPermit2Validation() public {
        ...
        assertFalse(
            executor.isAuthorizedForPermit2(testHash, emergencyOp),
            "Emergency operator should not be authorized for Permit2 initially"
        );
        ...
        assertTrue(
            executor.isAuthorizedForPermit2(testHash, emergencyOp),
            "Emergency operator should be authorized for Permit2 after addition"
        );
    }
    ...
    function test_ContractWalletERC165Check() public {
        ...
        assertTrue(
            contractWallet.supportsInterface(type(IERC165).interfaceId), "Contract wallet should support IERC165"
        );
        assertTrue(
            contractWallet.supportsInterface(type(IERC1271).interfaceId), "Contract wallet should support IERC1271"
        );
    ...
    function test_ContractWalletPermit2Integration() public {
        ...
        bytes32 permit2Hash = keccak256(
            abi.encodePacked(
                "PERMIT2_TRANSFER", address(contractWallet), address(executor), uint256(1000), block.timestamp + 3600
            )
        );
        ...
        assertTrue(
            executor.isAuthorizedForPermit2(permit2Hash, address(contractWallet)),
            "Contract wallet should be authorized for Permit2 operations"
        );
    ...
    function test_MultipleContractWalletsPermit2() public {
        ...
        assertFalse(
            executor.isAuthorizedForPermit2(hash2, address(contractWallet)), "Wallet 1 should not validate hash 2"
        );
